...
public class ContactosCovid {
	...
	private void procesarLinea(String[] datos) throws EmsInvalidTypeException, EmsInvalidNumberOfDataException, EmsDuplicateLocationException, EmsDuplicatePersonException {
		if (datos[0].equals("PERSONA")) {
            validarAnadirPersona(datos);
            return;
		}
		if (datos[0].equals("LOCALIZACION")) {
            validarAnadirLocalizacion(datos);
			return;
        }
		throw new EmsInvalidTypeException();
	}

	private void validarAnadirPersona(String[] datos) throws EmsInvalidNumberOfDataException, EmsDuplicatePersonException {
		if (datos.length != Constantes.MAX_DATOS_PERSONA) {
			throw new EmsInvalidNumberOfDataException("El número de datos para PERSONA es menor de 8");
		}
		this.poblacion.addPersona(this.crearPersona(datos));
	...
	private void  validarAnadirLocalizacion(String[] datos) throws EmsInvalidNumberOfDataException, EmsDuplicateLocationException {
		if (datos.length != Constantes.MAX_DATOS_LOCALIZACION) {
			throw new EmsInvalidNumberOfDataException("El número de datos para LOCALIZACION es menor de 6" );
		}
		PosicionPersona pp = this.crearPosicionPersona(datos);
